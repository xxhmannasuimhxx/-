...
// ============================================================
{
  ...
  const items = [
    ["崩れる前提でOK", "崩れるのは、あなたのせいじゃない。大事なのは、崩れても戻せる形を持っておくことでした。"],
    ["型はひとつ「主食＋たんぱく質＋色」", "麺の日はやめなくていい。大事なのは、やめることではなく「何を足すか」。"],
    ["食べさせる前に、まず光", "「食べない」の根っこは、リズムかもしれない。朝の光が、その日のスイッチを入れます。"],
  ];
  items.forEach((it, i) => {
    const y = 1.68 + i * 1.5;
  ...
  s.addNotes(
    ...
    "第1回のキーワード（崩れる前提／主食＋たんぱく質＋色／まず光）をそのまま使い、今日はその「戻し方」だと位置づける。"
  );
}
...
{
  const s = newSlide();
  header(s, "2. 朝", "朝ごはんに1品足す — のせるだけ・かけるだけ");

  const cols = [
    ["ごはんに のせる", ["卵（生卵・温泉卵）", "納豆", "しらす・ちりめんじゃこ", "鮭フレーク", "韓国のり"], AMBER],
    ["パンに のせる", ["スライスチーズ", "ハム・ツナ", "ピーナッツバター", "ヨーグルトを添える"], "C77E3A"],
    ["汁物に 入れる", ["乾燥わかめ", "豆腐", "冷凍のカット野菜", "卵を落とす"], SAGE],
  ];
  const cw = 3.71, gap = 0.4;
  cols.forEach((c, i) => {
    const x = M + i * (cw + gap);
  ...
  banner(s, 5.82, "切る・炒めるは要りません。のせる、かける、落とすだけ。1品足すのに30秒あれば十分です。", INK, W, 0.85, 16);

  s.addNotes(
    "いちばん実践しやすいスライド。ここは具体名をどんどん出す。\n" +
    "「調理しない」ことがポイント。買っておけば、朝は手を動かすだけで済む。"
  );
}
...
// ============================================================
{
  ...
  const tips = [
    ...
    ["飲みもので栄養を入れる", "牛乳、豆乳、飲むヨーグルト、具だくさんのスープ。噛まずに済むものなら、食べられない朝でも入ります。"],
  ];
  tips.forEach((t, i) => {
    const y = 1.62 + i * 1.62;

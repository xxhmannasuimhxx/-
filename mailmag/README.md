...
## 重要：最初に1回だけ「画面合わせ」が必要です

リザーブストックは外部向けのAPIを公開していません。そのため、この仕組みは
**ブラウザで管理画面を操作する**方式（Playwright）で作ってあります。
ログインURLや入力欄の名前は `mailmag/config/reservestock.json` に書いてありますが、
これは一般的な作りを想定した**初期値**です。実際の画面と違う場合は、
`inspect` コマンドで実画面を調べて、この JSON を直してください（手順は下記）。
